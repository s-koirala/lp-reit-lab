...
import { EmptyState, ProvenanceNote } from "@/components/ProvenanceNote";
    ...
    <AppShell>
      <header className="rule-b pb-6 mb-8">
        <p className="font-mono text-xs uppercase tracking-[0.22em] text-muted-foreground mb-2">
          Longitudinal market view
        </p>
        <h1 className="text-5xl md:text-6xl">
          A decade of North Side prices, rents, and liquidity.
        </h1>
        ...
        <ProvenanceNote className="mt-4" />
      </header>

      <ChartBlock
        title="Repeat-sales price index"
        subtitle="Jan 2015 = 100. All six submarkets, monthly."
        isEmpty={priceMerged.length === 0}
      >
        <ResponsiveContainer>
      ...
      <ChartBlock
        title="Rent index (3BR equivalent)"
        subtitle="Jan 2015 = 100."
        isEmpty={rentMerged.length === 0}
      >
        <ResponsiveContainer>
      ...
      <ChartBlock
        title="Market liquidity — median days on market"
        subtitle="36 months trailing. Lower = faster turnover."
        isEmpty={liqMerged.length === 0}
      >
        <ResponsiveContainer>
...
function ChartBlock({
  ...
  return (
    <section className="mb-10">
      ...
      <div className="bg-card rule-t rule-b p-4" style={{ height: 340 }}>
        {isEmpty ? <EmptyState label="No data for this view." /> : children}
      </div>
    </section>
  );
}
