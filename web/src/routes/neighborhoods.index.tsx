  ...
  return (
    <AppShell>
      <header className="rule-b pb-6 mb-8">
        <p className="font-mono text-xs uppercase tracking-[0.22em] text-muted-foreground mb-2">Submarket atlas</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-rule rule-t rule-b">
        {NEIGHBORHOODS.map((n) => {
          ...
          return (
            <Link key={n.slug} to="/neighborhoods/$slug" params={{ slug: n.slug }} className="bg-card p-6 hover:bg-secondary/40 transition-colors">
              <div className="flex items-start justify-between">
                <h2 className="text-3xl">{n.name}</h2>
                <span className="font-mono text-xs uppercase tracking-wider text-muted-foreground">{count} listings</span>
              </div>
