    ...
    <div className="min-h-screen bg-background paper-grain">
            ...
            <nav className="flex items-center gap-1">
              {NAV.map((n) => (
                <Link
                  ...
                  className="px-3 py-2 text-sm font-mono uppercase tracking-wider text-muted-foreground hover:text-foreground data-[status=active]:text-foreground data-[status=active]:border-b data-[status=active]:border-foreground transition-colors"
                >
                  {n.label}
                </Link>
              ))}
      ...
      <footer className="rule-t mt-16">
        <div className="mx-auto max-w-[1500px] px-6 py-8 text-xs text-muted-foreground flex flex-wrap gap-x-8 gap-y-2 justify-between">
          <span>
            {DATA_PROVENANCE.label} (vintage {DATA_PROVENANCE.asOf}). Estimates carry statistical
            uncertainty.
          </span>
          <span className="font-mono uppercase tracking-wider">
            Fair Housing compliant · market-demand framing only
          </span>
        </div>
      </footer>
    </div>
